...
def main() -> None:
    ...
    renderer=Renderer()
    writer=iio.get_writer(silent,fps=FPS,codec="libx264",quality=8,macro_block_size=None,ffmpeg_params=["-pix_fmt","yuv420p","-movflags","+faststart"])
    total=int(round(DURATION*FPS))
    for frame_index in tqdm(range(total),desc="Rendering Mars short"):
        writer.append_data(renderer.render(frame_index/FPS))
    writer.close()
    if SOUND:
        audio=OUTPUT/(BASENAME+"_soundtrack.wav")
        make_soundtrack(audio); mux_audio(silent,audio,final)
    else:
        silent.replace(final)
    if silent.exists(): silent.unlink()
    sheet=contact_sheet(final,PREVIEW_DIR/(BASENAME+"_contact_sheet.jpg"))
    manifest={"video":str(final),"subtitles":str(srt_path),"data_csv":str(csv_path),"method":str(json_path),"contact_sheet":str(sheet),"resolution":[W,H],"fps":FPS,"duration_s":DURATION,"sound":SOUND,"stats":STATS}
